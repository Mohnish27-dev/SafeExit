...
// The local-host guard and the throwaway-schema isolation both live in harness.mjs now.
// It refuses to boot at all against a non-local DATABASE_URL, and every table it creates
// is in a schema that is dropped at teardown — so there is nothing left to re-check here.
...
// ---------- A1. concurrent outing creation ----------
{
  ...
  const live = await OutingRequest.count({ where: { studentId: s.id, status: ['Pending','Approved','Forwarded','Out'] } });
...
// ---------- A2. concurrent leave creation ----------
{
  ...
  const created = res.filter(r => r.status === 201).length;
  const other = res.filter(r => r.status !== 201 && r.status !== 409);
  const live = await LeaveApplication.count({ where: { studentId: s.id, status: ['Pending','Approved','Forwarded','Out'] } });
...
// ---------- A3. cross-collection stacking ----------
{
  ...
  const liveO = await OutingRequest.count({ where: { studentId: s.id, status: ['Pending','Approved','Forwarded','Out'] } });
  const liveL = await LeaveApplication.count({ where: { studentId: s.id, status: ['Pending','Approved','Forwarded','Out'] } });
...
if (okWindow) {
  ...
  const logs = await ScanLog.count({ where: { studentId: s.id, direction: 'OUT' } });
  ...
  const inLogs = await ScanLog.count({ where: { studentId: s.id, direction: 'IN' } });
  const after2 = await User.findByPk(s.id);
  const pass = await OutingRequest.findOne({ where: { studentId: s.id }, order: [['createdAt', 'DESC']] });
...
// ---------- A7. exit with NO approved pass ----------
{
  ...
  const st = await User.findByPk(s.id);
...
// ---------- A9. past departure time accepted at create ----------
{
  ...
  const r = await api('/api/scan', { method: 'POST', token: T.guard, body: { student: s._id.toString(), direction: 'OUT' } });
  const doc = c.body?._id ? await OutingRequest.findByPk(c.body._id) : null;
...
// ---------- A10. leave pass double-use across collections at the gate ----------
{
  ...
  const ou = await OutingRequest.create({ studentId: s.id, destination: 'M', purpose: 'p',
  ...
  const lv2 = await LeaveApplication.findByPk(lv.id);
  const ou2 = await OutingRequest.findByPk(ou.id);
