...
const logging = () =>
  process.env.SQL_LOG === 'true' ? (sql, ms) => console.log(`[sql ${ms}ms] ${sql}`) : false;
...
const buildOptions = () => {
  ...
  return {
    ...
    retry: {
      max: 2,
      match: [/ECONNRESET/, /ECONNREFUSED/, /Connection terminated unexpectedly/],
    },
  };
};
...
const getSequelize = () => {
  if (!sequelize) {
    ...
    sequelize = url ? new Sequelize(url, buildOptions()) : new Sequelize(buildOptions());
  }
  return sequelize;
};
...
const connectPostgres = async () => {
  ...
  try {
    ...
    if (Number(info.max_conn) > 0 && poolConfig().max > Number(info.max_conn) / 2) {
      console.warn(
