    ...
    return (
      <section className="sd-luxe-panel sd-luxe-rise rounded-4xl p-4 sm:p-5 shadow-md">
        <div className="flex items-center justify-between">
          <h3 className="sd-title sd-title-sm">Auto Approved</h3>
          <button onClick={onViewAll} className="px-3 py-2 rounded bg-indigo-600 text-white">View All</button>
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-4">
          {approved.slice(0, 6).map((s, i) => (
            <div key={s.id} className="sd-card-soft rounded-xl p-3 flex flex-col items-center text-center">
              ...
              <span className="bg-orange-100 text-orange-600 text-[10px] font-bold px-2 py-1 rounded-md">Not Returned</span>
            </div>
          ))}
        </div>
      </section>
    ...
    <section className="sd-luxe-panel sd-luxe-rise mt-6 rounded-4xl p-6 sm:p-7 shadow-xl">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onClear} className="px-3 py-2 rounded bg-rose-600 text-white">Clear All</button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-2 rounded border disabled:opacity-40">Prev</button>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-2 rounded border disabled:opacity-40">Next</button>
          </div>
