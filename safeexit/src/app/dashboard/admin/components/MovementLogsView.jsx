    ...
    <section className="space-y-4 sm:space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/70 bg-white/80 px-4 py-3 shadow-sm backdrop-blur sm:rounded-3xl sm:px-5 sm:py-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-base font-bold text-slate-900 sm:text-lg">Movement Logs</h2>
            <p className="text-xs text-slate-600 sm:text-sm">{visible.length} gate scans</p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-3xl border border-dashed border-slate-200 bg-white py-16 text-center">
          ...
          <p className="mt-3 font-semibold text-slate-700">No movement logs yet</p>
          <p className="text-sm text-slate-400">Gate scans by guards will appear here in real time.</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm sm:rounded-3xl">
          {visible.map((l, i) => {
            ...
            return (
              <div
                key={l._id}
                className={`flex flex-wrap items-center justify-between gap-3 px-4 py-3 sm:gap-4 sm:px-5 sm:py-4 ${i !== 0 ? "border-t border-slate-100" : ""}`}
              >
                <div className="flex min-w-0 items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 text-sm font-bold text-slate-600 sm:h-11 sm:w-11">
                    {getInitials(st.name)}
                  </div>
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold text-slate-900">{st.name || "Unknown"}</p>
                ...
                <div className="flex min-w-0 flex-wrap items-center gap-2 sm:gap-3">
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${
                      isOut ? "bg-sky-100 text-sky-700" : "bg-emerald-100 text-emerald-700"
                    }`}
                  >
