...
export default function DepartmentDashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50/40 to-teal-50/30 text-slate-900 pb-16">
      <div className="mx-auto flex w-full max-w-4xl flex-col px-4 py-6 sm:px-6">
        ...
        <header className="flex items-center justify-between gap-3 rounded-3xl border border-white/80 bg-white/90 px-4 py-4 shadow-sm backdrop-blur sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-500 text-white shadow-lg">
              <DeptIcon className="h-6 w-6" />
            </div>
            <div className="min-w-0">
              <p className="text-[11px] font-bold uppercase tracking-wider text-emerald-600">{t("desk")}</p>
        ...
        {pushPermission === "default" && !pushBannerDismissed && (
          <div className="mt-4 flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/80 px-5 py-4 text-emerald-800 shadow-sm">
            <Bell className="h-5 w-5 shrink-0 animate-bounce text-emerald-500" />
            <div className="min-w-0 flex-1">
              <p className="font-bold">{t("enablePush")}</p>
              <p className="text-sm text-emerald-700">{t("enablePushHint")}</p>
            </div>
            <button
              onClick={handleEnablePush}
              disabled={pushEnabling}
              className="shrink-0 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white shadow-md transition hover:bg-emerald-700 active:scale-95 disabled:opacity-60"
            >
              {pushEnabling ? t("enabling") : t("enable")}
            </button>
            <button onClick={() => setPushBannerDismissed(true)} className="shrink-0 rounded-xl p-2 text-emerald-400 transition hover:bg-emerald-100" title={t("dismiss")}>
              <X className="h-4 w-4" />
            </button>
          </div>
        )}
