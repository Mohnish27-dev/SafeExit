...
export default function CaretakerLoginPage() {
	...
	const loginCaretakerAccount = async (loginId, pin) => {
		...
		if (data.role !== "Caretaker") {
			throw new Error(
				"This account is not authorized for caretaker access.",
			);
		}
		return data;
	};
	...
	const submitQuickSetup = async () => {
		...
		try {
			const profile = JSON.parse(
				localStorage.getItem("safeexit_caretaker_profile"),
			);
			// Lock the admin PIN behind the Quick Login PIN
			await quick.setQuickPin(
				quickPin,
				pendingSecret,
				profile.caretakerId,
			);
								...
								>
									<div className="text-center mb-6">
										...
										<p className="text-sm text-slate-500 mt-1">
											Sign in with the Caretaker ID and
											PIN issued by your administrator.
										</p>
									</div>
									...
									<button
										type="submit"
										disabled={isProcessing}
										className="w-full mt-6 flex items-center justify-center gap-2 py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold text-sm shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 active:scale-[0.98] transition-all cursor-pointer disabled:opacity-70"
									>
										{isProcessing ? (
											"Signing in…"
										) : (
								...
								<div className="space-y-6 animate-fade-in-up flex flex-col items-center text-center">
									...
									<div>
										...
										<p className="text-sm text-slate-500 mt-2 max-w-sm mx-auto">
											Create a 4-digit PIN to sign in fast
											next time. Add your fingerprint or
											face for even quicker access —
											it&apos;s optional.
										</p>
									</div>
