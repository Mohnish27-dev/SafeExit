...
// Department accounts have no email — the normalized Department ID IS the backend loginId
const buildDepartmentLoginId = (departmentId) =>
  departmentId.trim().toLowerCase().replace(/\s+/g, "");

// Own localStorage namespace so it never collides with other role logins
const quick = makeQuickLogin({
  pinKey: "safeexit_quick_pin_department",
  labelKey: "safeexit_quick_label_department",
  profileKey: "safeexit_department_profile",
  webauthnKey: "safeexit_webauthn_registered_department",
});
...
export default function DepartmentLoginPage() {
  ...
  const [formData, setFormData] = useState({ departmentId: "", pin: "" });
  ...
  useEffect(() => {
    const profileRaw = localStorage.getItem("safeexit_department_profile");
  ...
  const validateStep1 = () => {
    if (!formData.departmentId.trim() || formData.pin.trim().length < 4) {
      setErrorMsg("Please enter your Department ID and PIN.");
      return false;
    }
    setErrorMsg("");
    return true;
  ...
  // Department accounts are admin-provisioned, not self-registered
  const loginDepartmentAccount = async (loginId, pin) => {
    ...
    if (!res.ok) {
      throw new Error(
        res.status === 401
          ? "Account not found or PIN incorrect. Ask your administrator to provision or reset your department account."
          : data.message || "Login failed."
      );
    }
    // Defence in depth: keep non-department accounts off the department dashboard
  ...
  const persistDepartmentSession = (profile) => {
    setStoredUser({
      name: profile.fullName,
      role: "department",
      roleLabel: "Department",
      id: profile.departmentId,
      ...(profile.managedDepartment ? { managedDepartment: profile.managedDepartment } : {}),
    });
  };
  ...
  const enrollBiometric = async (token) => {
    ...
    localStorage.setItem("safeexit_webauthn_registered_department", "true");
  };
  ...
  const submitStep1 = async (e) => {
    ...
    setErrorMsg("");
    try {
      const loginId = buildDepartmentLoginId(formData.departmentId);
      const data = await loginDepartmentAccount(loginId, formData.pin);
      ...
      const profile = {
        ...
        fullName: data.name,
        managedDepartment: data.managedDepartment,
      };
      localStorage.setItem("safeexit_department_profile", JSON.stringify(profile));
      setStoredProfile(profile);
      persistDepartmentSession(profile);
  ...
  const submitQuickSetup = async () => {
    ...
    if (quickPin !== confirmQuickPin) {
      setErrorMsg("The PreINs you entered do not match.");
      return;
    }
    setIsProcessing(true);
    setErrorMsg("");
    try {
      const profile = JSON.parse(localStorage.getItem("safeexit_department_profile"));
      // Lock the admin PIN behind the Quick Login PIN
      await quick.setQuickPin(quickPin, pendingSecret, profile.departmentId);
  ...
  const handlePinLogin = async (e) => {
    ...
    try {
      ...
      const loginId = buildDepartmentLoginId(storedProfile.departmentId);
      const data = await loginDepartmentAccount(loginId, secret);
      sessionStorage.setItem("safeexit_token", data.token);

      persistDepartmentSession(storedProfile);
      router.push("/dashboard/department");
    } catch (err) {
              ...
              {onboardingStep === 1 && (
                <form onSubmit={submitStep1} className="space-y-4 animate-fade-in-up">
                  <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-900">Department Login</h2>
                    <p className="text-sm text-slate-500 mt-1">Sign in with the Department ID and PIN issued by your administrator.</p>
                  </div>
